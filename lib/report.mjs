...
import { ALL_CHECKS, checkById, checklistItems, familyById, familySections } from './checks/index.mjs';
...
function markdown(run) {
  ...
  // Таблица на семейство, а не одна общая: техническое SEO и содержание страницы правят
  // разные люди, и в одной простыне пункт про цену терялся среди метатегов.
  for (const { family, items } of familySections()) {
    out.push('');
    out.push(`### ${family.title}`);
    if (family.about) {
      out.push('');
      out.push(`_${family.about}_`);
    }
    out.push('');
    out.push(
    ...
    for (const item of items) {
      ...
      const anchor = sectionAnchor.get(`${item.family}::${item.checklist}`);
  ...
  if (!fresh.length) {
    ...
    for (const [checkId, list] of byCheck) {
      ...
      const family = familyById(check?.family ?? list[0].family);
      out.push(
        `- ${family.short}: **${check?.checklist ?? checkId}** (\`${checkId}\`): ${list.length} — страницы: ${where}`,
      );
    }
  }
  ...
  // Находки тоже разложены по семействам: раздел на каждое, внутри — пункты чеклиста.
  for (const { family, items } of familySections()) {
    const familyFindings = items.flatMap((i) => i.checks.flatMap((id) => byCheck.get(id) ?? []));
    out.push('');
    out.push(`## Находки: ${family.title} — ${familyFindings.length}`);
    if (!familyFindings.length) {
      out.push('');
      out.push('Находок нет.');
      continue;
    }
...
function slackText(run) {
  ...
  for (const f of run.findings) {
    ...
    if (!groups.has(key)) {
      groups.set(key, {
        ...
        family: familyById(f.family ?? checkById(f.check_id)?.family).short,
        count: 0,
        slugs: new Set(),
        fresh: 0,
      });
  ...
  if (important.length) {
    ...
    for (const g of important.slice(0, 12)) {
      ...
      out.push(`• ${mark} ${g.severity} ${g.family} · ${g.checklist}: ${g.count} — ${where}`);
    }
  ...
  if (minor.length) {
    ...
    for (const g of minor) {
      out.push(`• P3 ${g.family} · ${g.checklist}: ${g.count} (${g.slugs.size} стр.)`);
    }
  }
...
export function briefText(run) {
  ...
  if (groups.size) {
    out.push('');
    out.push('По видам проблемы (важность · семейство · пункт · находок · страниц):');
    const sorted = [...groups.values()].sort(
      (a, b) => a.severity.localeCompare(b.severity) || b.count - a.count,
    );
      ...
      const where =
        g.slugs.size === t.pages
          ...
          : g.slugs.size > 4
            ...
            : [...g.slugs].join(', ');
      out.push(
        `- ${g.severity} ${g.family} · ${g.checklist}: ${g.count}${g.fresh ? ` (новых ${g.fresh})` : ''} — ${where}`,
      );
