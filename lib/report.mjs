...
function markdown(run) {
  ...
  for (const item of checklistItems()) {
    const cells = pages.map((page) => {
      ...
      // Пункт, у которого часть подпроверок невозможна, не должен выглядеть полностью
      // проверенным: иначе ✅ обещает больше, чем бот действительно посмотрел.
      const partial = statuses.length > 1 && statuses.includes('na');
  ...
  if (hasPartial) {
    out.push('');
    out.push('`*` пункт проверен частично — часть его проверок невозможна, см. раздел «Не проверено».');
  }
