...
function occurrenceLine(f, prefix = '', urlBySlug = null) {
  const entity = String(f.entity ?? '').slice(prefix.length);
  const where = entity && entity !== '—' ? ` · ${entity}` : '';
  const actual = f.actual ? ` — ${f.actual}` : '';
  return `- **${f.severity}** ${slugLink(f.slug, urlBySlug)}${where}${actual} · ${ageMark(f)} \`${f.fingerprint}\``;
}

function occurrenceRow(f, prefix = '', urlBySlug = null) {
  const entity = String(f.entity ?? '').slice(prefix.length);
  return `| ${f.severity} | ${slugLink(f.slug, urlBySlug)} | ${escapeCell(clip(entity, 120))} | ${escapeCell(clip(f.actual ?? '', 100))} | ${ageMark(f)} |`;
}
  ...
  for (const item of checklistItems()) {
    ...
    for (const checkId of item.checks) {
      ...
      for (const group of findingGroups(list)) {
        ...
        //
        // Оформлено alert-блоком GitHub: он сам рисует жёлтую рамку и иконку
        // предупреждения. Своей заливкой это не сделать — GitHub вырезает CSS из
        // markdown, поэтому «рамка + жёлтый фон» существуют только в этом виде.
        if (first.fix) {
          out.push('');
          out.push('> [!WARNING]');
        ...
        if (group.length > foldAbove) {
          ...
          out.push('| --- | --- | --- | --- | --- |');
          for (const f of group) out.push(occurrenceRow(f, prefix, urlBySlug));
          out.push('');
          out.push('</details>');
        } else {
          for (const f of group) {
            out.push(occurrenceLine(f, prefix, urlBySlug));
