...
function occurrenceTable(group, prefix = '', urlBySlug = null, { evidence = true } = {}) {
  ...
  const columns = [
    ...
    { head: 'блок на странице', value: (f) => escapeCell(clip(f.block ?? '', 60)) },
...
function occurrenceData(group, prefix = '', urlBySlug = null, { evidence = true } = {}) {
  ...
  const columns = [
    ...
    { head: 'блок на странице', value: (f) => clip(f.block ?? '', 60) },
...
export function html(run) {
  ...
  for (const item of checklistItems()) {
    for (const checkId of item.checks) {
      const list = byCheck.get(checkId) ?? [];
      if (!list.length) continue;
      // Одна проверка может дать несколько групп разного вида (у «Нет ошибок в консоли» —
      // это и console.error, и упавшие запросы; у ссылок — битые и nofollow). Если групп
      // больше одной, к названию добавляем вид проблемы, иначе в отчёте идут несколько
      // карточек с одинаковым заголовком и их не различить.
      const groups = findingGroups(list);
      // Вид проблемы для различителя: общий префикс entity («битая», «nofollow внутри»)
      // читается лучше всего. Если его нет (у консоли entity — это текст ошибки) или он
      // совпал у двух групп (битая P1 и битая P2), берём «ожидалось» — оно точнее.
      const prefixKinds = groups.map((g) => commonEntityPrefix(g).replace(/[\s:]+$/, '').trim());
      const kindCount = {};
      for (const k of prefixKinds) if (k) kindCount[k] = (kindCount[k] ?? 0) + 1;
      for (let gi = 0; gi < groups.length; gi++) {
        const group = groups[gi];
        const first = group[0];
        const prefix = commonEntityPrefix(group);
        let name = checkLabel(checkId);
        if (groups.length > 1) {
          const pk = prefixKinds[gi];
          const kind = (pk && kindCount[pk] === 1 ? pk : '') || first.expected || first.actual || pk || '';
          if (kind) name += ` — ${clip(String(kind), 50)}`;
        }
        findGroups.push({
  ...
  const findingCells = (f) => {
    const cols = [
      ...
      ['блок на странице', clip(String(f.block ?? ''), 60)],
...
.grp>summary::after{content:"›";font-size:20px;color:var(--ink-soft);transform:rotate(90deg);transition:transform .15s;flex:none}
...
.grp .title{font-weight:640;flex:1 1 auto;min-width:0}
.grp .cnt{font-size:12px;color:var(--ink-soft);font-variant-numeric:tabular-nums;flex:none;white-space:nowrap;text-align:right;margin-left:12px}
...
.copybtn{border:1px solid var(--line-strong);background:var(--panel-2);color:var(--ink-soft);border-radius:7px;padding:5px 11px;font:inherit;font-size:12px;font-weight:600;cursor:pointer;margin-bottom:10px}
...
.topnav{position:fixed;top:14px;right:16px;display:flex;gap:8px;z-index:5}
.navbtn{background:var(--panel);border:1px solid var(--line-strong);color:var(--ink-soft);border-radius:8px;padding:6px 10px;font:inherit;font-size:12px;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center}
...
// Значения в D.findGroups уже экранированы под HTML (&lt; &amp; …). Для plain-text
// разметки Jira их надо вернуть в исходный вид, иначе в задаче будет «&lt;a href».
function unesc(s){ const t=document.createElement('textarea'); t.innerHTML=String(s??''); return t.value; }
// Разметка таблицы Jira (wiki): ||шапка|| и |ячейки|. Вертикальную черту и переносы
// внутри ячейки экранируем/схлопываем — иначе таблица разъедется.
function jiraMarkup(head, rows, title){
  const esc = s => String(s??'').replace(/\\|/g,'\\\\|').replace(/\\r?\\n/g,' ');
  const out = [];
  if(title) out.push('h3. '+title, '');
  out.push('||'+head.map(esc).join('||')+'||');
  for(const r of rows) out.push('|'+r.map(esc).join('|')+'|');
  return out.join('\\n');
}
// HTML-таблица для буфера: редактор Jira Cloud вставляет её как настоящую таблицу.
// Значения head/rows уже экранированы под HTML — вставляем как есть.
function jiraHtml(head, rows, title){
  let h = title ? '<p><b>'+title+'</b></p>' : '';
...
function legacyCopy(text){
  ...
  ta.select(); let ok=false; try{ ok=document.execCommand('copy'); }catch(e){} document.body.removeChild(ta);
  return ok;
}
// Копируем и настоящую таблицу (text/html — для Jira Cloud), и wiki-разметку
// (text/plain — фолбэк для полей с wiki и Jira DC). Так вставка не ломает формат.
async function copyForJira(head, rows, title, btn){
  const html = jiraHtml(head, rows, title);
  const text = jiraMarkup(head.map(unesc), rows.map(r=>r.map(unesc)), title);
  try{
    if(navigator.clipboard && window.ClipboardItem){
      ...
      flash(btn, 'Скопировано ✓'); return;
    }
    if(navigator.clipboard){ await navigator.clipboard.writeText(text); flash(btn,'Скопировано ✓'); return; }
    flash(btn, legacyCopy(text) ? 'Скопировано ✓' : 'Не удалось скопировать');
  }catch(e){ flash(btn, legacyCopy(text) ? 'Скопировано ✓' : 'Не удалось скопировать'); }
}
function renderGroups(){
  ...
  for(const g of groups){
    ...
    d.innerHTML =
      ...
      '<div><button class="copybtn" type="button">📋 Копировать для Jira</button></div>'+
      '<div class="tblwrap"><table><thead><tr>'+th+'</tr></thead><tbody>'+trs+'</tbody></table></div></div>';
    // Копируем именно то, что видно в группе с учётом фильтра: head и отфильтрованные rows.
    const title = g.severity+' · '+g.name.replace(/<[^>]+>/g,'')+' — '+rows.length+' наход.';
    d.querySelector('.copybtn').addEventListener('click', e=>{
      e.preventDefault(); e.stopPropagation();
      copyForJira(head, rows, title, e.currentTarget);
    });
    box.appendChild(d);
  }
...
<body>
<div class="topnav">
  <a class="navbtn" href="settings.html">⚙ Настройки</a>
  <button class="navbtn" id="themebtn">◐ тема</button>
</div>
<div class="wrap">
  <div class="masthead">
    <h1><a href="index.html">SEO Pedant</a></h1><span class="run">${escHtml(s.runId)}</span>
  </div>
