...
async function collectPage(runId, page, log) {
  ...
  // Печатаем только то, что стоит внимания: на сотне страниц строка на каждую — это
  // сотня строк в чужом контексте, и обычное «200 OK» в ней ничего не сообщает.
  const quiet = !res.error && status === 200 && !res.redirected;

  return { meta, html: res.html, shape, quiet };
}
...
async function main() {
  ...
  for (const page of selected) {
    const result = await collectPage(runId, page, log);
    if (result.quiet && !values.verbose) quietPages++;
    else log(`  ${page.slug}: ${result.shape}`);
    collected.push(result);
  }
  if (quietPages) log(`  ${quietPages} стр.: 200 OK без редиректов`);
  ...
  if (!values['static-only']) {
    ...
    // Lighthouse — самая долгая часть сбора (десятки секунд на страницу), поэтому он
    // гоняется только там, где проверка перформанса включена. Отключил её для типа
    // страниц — метрики по ним и не собираются.
    const wantsLighthouse = (meta) =>
      !values['no-lighthouse'] &&
      !configForType(meta.type ?? 'unknown').checks.disabled.includes('performance');
    browserRuns = await collectBrowser(runId, collected, {
      log,
      lighthouse: wantsLighthouse,
  ...
  writeArtifact(runId, SITE_SLUG, 'collect.json', {
    ...
    mode: values['static-only'] ? 'static' : browserRuns?.lighthouse.ran ? 'full' : 'browser',
    lighthouse: browserRuns?.lighthouse ?? { ran: 0, skipped: 0 },
