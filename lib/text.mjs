...
/**
 * Окно текста вокруг подстроки — цитата для находки, у которой доказательство это само
 * место в тексте. В отличие от sentenceWith, найденное слово гарантированно попадает в
 * цитату: «предложение с начала» на длинных перечислениях обрезалось до самой цены, и в
 * отчёт уезжал кусок хлебных крошек вместо доказательства.
 */
export function quoteAround(text, needle, max = 220) {
  const t = collapse(text);
  const target = collapse(needle);
  const at = t.toLowerCase().indexOf(target.toLowerCase());
  if (at < 0) return null;
  const half = Math.max(20, Math.floor((max - target.length) / 2));
  const from = Math.max(0, at - half);
  const to = Math.min(t.length, at + target.length + half);
