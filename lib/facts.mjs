...
import { collapse, wordCount } from './text.mjs';
 ...
 * @param {{ownDomains?: string[]}} [options] — наши домены из config/scope.json.
 *   Ссылка на любой из них внутренняя, остальные внешние. Домен самой страницы
 *   всегда свой, даже если его нет в списке.
 */
export function htmlFacts(html, baseUrl, options = {}) {
  const source = String(html ?? '');
  ...
  const ownSites = new Set([pageSite, ...(options.ownDomains ?? []).map(site)]);
  ...
  $('a[href]').each((_, el) => {
    ...
    links.push({
      ...
      text: collapse($el.text()),
      site: linkSite,
      internal: Boolean(linkSite) && ownSites.has(linkSite),
      external: Boolean(linkSite) && !ownSites.has(linkSite),
    });
  });
