...
export function keywordsFor(page) {
  if (Array.isArray(page.keywords) && page.keywords.length) {
    return {
      ...
      topic: null,
      entity: null,
      all: page.keywords.map((w) => ({ word: w, stem: stem(w), role: 'custom' })),
    };
  }
  ...
  const topicWord = TOPIC_BY_SEGMENT[firstSegment] ?? null;
  const topic = topicWord ? { word: topicWord, stem: stem(topicWord), role: 'topic' } : null;

  let source = 'URL';
  let words = nameTokens(page.url).map(translit);
  ...
  const entities = words.map((word) => ({ word, stem: stem(word), role: 'entity' }));
  return {
    source,
    topic,
