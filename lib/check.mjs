...
function loadFacts(runId, slug, site, links, ownDomains) {
  ...
  return {
    ...
    finalUrl: parsedBase,
    html: rawHtml ? htmlFacts(rawHtml, base, { ownDomains }) : null,
    dom: domHtml ? htmlFacts(domHtml, base, { ownDomains }) : null,
...
function main() {
  ...
  // Наши домены берём из текущего конфига, а не из прогона: добавили домен —
  // перепрогон проверок по сохранённым страницам сразу учтёт его.
  ...
  const facts = slugs
    .map((slug) => loadFacts(runId, slug, site, links, scopeConf.own_domains))
    .filter(Boolean);
