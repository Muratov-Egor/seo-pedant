 ...
 */
function readHistory(runId, keepFrom = null) {
  const rows = [];
  let dropped = 0;
  if (existsSync(HISTORY_FILE)) {
    for (const line of readFileSync(HISTORY_FILE, 'utf8').split('\n')) {
      ...
      try {
        ...
        if (!row.fingerprint || !row.run) continue;
        // Отсечённые строки не попадут и в перезапись файла — так ретеншен и работает.
        if (keepFrom && row.date && row.date < keepFrom) {
          dropped++;
          continue;
        }
        rows.push(row);
...
/**
 * Сколько дней истории хранить. Всё, что старше, выбрасывается при следующем прогоне:
 * файл лежит в репозитории и дописывается каждый прогон, поэтому расти без предела ему
 * нельзя. Цена ретеншена: находка, вернувшаяся после месяца тишины, считается новой —
 * прошлых её появлений в истории уже нет.
 */
...
function main() {
  ...
  const history = readHistory(runId, minusDays(runDate, HISTORY_KEEP_DAYS));

  const scopeConf = scope();
  // Ключевые слова и label читаем из текущего конфига, а не из артефактов прогона:
  // поправил label или задал keywords — перепрогон по сохранённым страницам это учтёт.
  const configBySlug = new Map(configuredPages().map((p) => [p.slug, p]));
  const facts = slugs
    .map((slug) => loadFacts(runId, slug, site, links))
    .filter(Boolean);
  ...
  for (const f of facts) {
    ...
    const ctx = {
      ...
      keywords: keywordsFor({
        url: f.page.url,
        keywords: configBySlug.get(f.page.slug)?.keywords,
        label: configBySlug.get(f.page.slug)?.label ?? f.page.label,
      }),
      config: typeConf,
    };
    ...
    for (const check of pageChecks) {
      if (disabled.has(check.id)) {
        verdicts.push({
          check_id: check.id,
          status: SKIP,
          reason: `отключена в ${typeConf.checks.disabledSource[check.id]}`,
        });
        continue;
      }
      ...
      verdicts.push({
        ...
        partial: Boolean(verdict.partial),
  ...
  // Построчно — худшие страницы, а не все. Строка на каждую из сотни забивает вывод и
  // чужой контекст, ничего в них не сообщая: что именно сломано, видно в сводке отчёта,
  // сгруппированной по виду проблемы. Полный список: --verbose.
  const PAGE_LINES = 10;
  const summaries = [];
  for (const page of pages) {
    if (page.blocked) {
    ...
    summaries.push({ slug: page.slug, counts, weight: counts.fail * 10 + counts.warn });
  }
  const shown = values.verbose
    ? summaries
    : [...summaries].sort((a, b) => b.weight - a.weight).slice(0, PAGE_LINES);
  for (const { slug, counts } of shown) {
    console.log(
      `  ${slug}: ✅ ${counts.pass}  ❌ ${counts.fail}  ⚠️ ${counts.warn}  ⏭ ${counts.skip}  – ${counts.na}`,
    );
  }
  const hidden = summaries.length - shown.length;
  if (hidden > 0) {
    const clean = summaries.filter((s) => !s.weight).length;
    console.log(
      `  …и ещё ${hidden} стр.${clean ? ` (без нарушений и замечаний — ${clean})` : ''}; полный список — --verbose`,
    );
  }
  ...
  if (scopeConf.development.enabled) {
    console.log(
      `Режим разработки: внутренние ссылки проверялись частично — первые ${scopeConf.development.internal_links_per_page} на страницу (config/scope.json).`,
    );
  }
  console.log(
  ...
  if (history.dropped) {
    console.log(
      `История: ${history.dropped} строк старше ${HISTORY_KEEP_DAYS} дней выброшено${values.dry ? ' (в dry-режиме файл не перезаписан)' : ''}.`,
    );
  }
