...
/**
 * @param {boolean | ((meta: object) => boolean)} [options.lighthouse] — гонять ли Lighthouse.
 *   Предикат вызывается на каждую страницу: собирать метрики там, где проверка
 *   перформанса отключена, значит тратить минуты на страницу впустую.
 */
export async function collectBrowser(
  runId,
  collected,
  { log = () => {}, lighthouse = true, verbose = false } = {},
) {
  const port = await freePort();
  const browser = await chromium.launch({ args: [`--remote-debugging-port=${port}`] });
  log(`  браузер запущен, CDP-порт ${port}`);
  const lighthouseFor = typeof lighthouse === 'function' ? lighthouse : () => Boolean(lighthouse);
  const lighthouseRuns = { ran: 0, skipped: 0 };
  let quietPages = 0;
  ...
  try {
    for (const { meta } of collected) {
      ...
      // Тихая страница — та, где смотреть нечего: DOM снят, консоль чистая, по ширине
      // не вылезает. Такие идут в счётчик, а не строкой в вывод.
      const quiet =
        !rendered.navigationError &&
        !rendered.errors &&
        !rendered.failed &&
        !mobile.error &&
        !mobile.overflowPx;
  ...
  if (quietPages) log(`  ${quietPages} стр.: DOM снят, консоль чистая, по ширине не вылезает`);
  if (lighthouseRuns.skipped) {
    log(
      `  Lighthouse пропущен на ${lighthouseRuns.skipped} стр. — проверка перформанса отключена` +
        `${lighthouseRuns.ran ? `, прогнан на ${lighthouseRuns.ran}` : ''}`,
    );
  }
  return { lighthouse: lighthouseRuns };
}
