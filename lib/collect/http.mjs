...
// Заголовки обычного перехода по ссылке в браузере. sec-fetch-* и
// upgrade-insecure-requests стоят здесь не для красоты: facebook.com отвечает на запрос
// без них 400 Bad Request, и три живые ссылки на наши страницы в соцсети приезжали в
// отчёт битыми. С этими заголовками те же адреса отдают 200 и на HEAD, и на GET.
...
export const REFUSED_STATUSES = new Set([202, 400, 401, 403, 405, 418, 429, 501, 999]);
  ...
  for (const method of ['HEAD', 'GET']) {
    try {
      const res = await fetch(url, {
        method,
        redirect: 'follow',
        // Заголовки те же, что у перехода по ссылке: подмена accept на */* выдавала в
        // нас робота. Картинки и PDF всё равно проходят — в accept есть */*;q=0.8.
