  ...
  run(f) {
    ...
    if (headings.length === 0) {
      return judge(
        [
          {
            entity: 'заголовки',
            expected: 'есть заголовки',
            actual: 'на странице нет ни одного h1–h6',
            fix: 'Разметить блоки страницы заголовками h1–h6.',
          },
        ],
        this.severity,
      );
    ...
    if (headings[0].level !== 1) {
      findings.push({
        ...
        severity: 'P3',
        fix: 'Сделать первым заголовком страницы <h1>.',
      });
    }
    ...
    for (const h of headings.slice(1)) {
      if (h.level > previous + 1) {
        findings.push({
          ...
          fix: 'Не пропускать уровни: понизить заголовок до следующего по порядку.',
        });
      }
      previous = h.level;
    }
