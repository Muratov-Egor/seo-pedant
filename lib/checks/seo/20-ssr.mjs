  ...
  run(f, ctx) {
    ...
    if (dom.h1[0] && !raw.h1[0]) {
      findings.push({
        ...
        fix: 'Отдавать h1 сразу в HTML от сервера, а не дорисовывать его на клиенте.',
      });
      ...
      findings.push({
        ...
        fix: 'Привести h1 от сервера к тому, что видит пользователь после отрисовки.',
      });
    }
    ...
    if (dom.title && (!raw.title || !sameIgnoringNumbers(dom.title, raw.title))) {
      findings.push({
        ...
        severity: 'P2',
        note: 'Числа при сравнении не учитывались — расхождение не в живых ценах.',
        fix: 'Отдавать в HTML от сервера тот же title, что проставляется после отрисовки.',
      });
    }
    ...
    if (ratio < min) {
      findings.push({
        ...
        fix: 'Рендерить основной текст на сервере — сейчас большая его часть появляется только в браузере.',
      });
    }
