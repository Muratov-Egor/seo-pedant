        ...
        for (const field of FIELDS) {
          ...
          for (const [self, other] of [
            ...
            findings.get(self.page.slug).push({
              ...
              fix: `Написать для этой страницы свой ${field.label} — сейчас он дословно как у другой страницы.`,
            });
          }
        }
        ...
        if (sim >= ctx.thresholds.duplicate_text_pct) {
          for (const [self, other] of [
            ...
            findings.get(self.page.slug).push({
              ...
              fix: 'Переписать общую часть текста под конкретное направление, а не подставлять название в один шаблон.',
            });
          }
        }
      }
