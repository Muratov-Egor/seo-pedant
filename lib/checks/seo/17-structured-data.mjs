  ...
  run(f) {
        ...
        [
          {
            ...
            actual: 'тега нет',
            fix: 'Добавить на страницу <script type="application/ld+json"> с разметкой Schema.org.',
          },
        ],
        this.severity,
      );
    ...
    scripts.forEach((script, index) => {
      if (script.error) {
        findings.push({
          ...
          severity: 'P1',
          fix: 'Поправить JSON внутри ld+json — сейчас он не разбирается, и разметка не читается.',
        });
      }
    });
    ...
    if (objects.length && !complete) {
      ...
      findings.push({
        ...
        fix: `Дописать в ld+json поля: ${best.missing.join(', ')}.`,
      });
    }
