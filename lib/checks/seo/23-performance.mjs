    ...
    for (const [key, label] of FORM_FACTORS) {
      ...
      if (run.error) {
        findings.push({
          ...
          severity: 'P3',
          fix: 'Проверить, что Lighthouse и браузер поднимаются в этом окружении.',
        });
        continue;
      }
      ...
      if (run.score < min) {
        ...
        findings.push({
          ...
          fix: 'Начать с худшей метрики из подтверждения (обычно LCP или TBT); дёргаться из-за 1–2 баллов не нужно.',
        });
      }
    }
