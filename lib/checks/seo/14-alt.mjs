    ...
    f.html.images.forEach((img, index) => {
      ...
      if (!img.hasAlt) {
        findings.push({
          ...
          fix: `Добавить у <img> атрибут alt с описанием картинки в ${min}–${max} словах.`,
        });
        return;
      }
      ...
      if (!img.alt) {
        findings.push({
          ...
          actual: 'alt пустой',
          severity: 'P3',
          fix: `Заполнить alt описанием в ${min}–${max} словах; пустой alt оставлять только у декоративных картинок.`,
        });
        return;
      }
      ...
      if (img.altWords < min || img.altWords > max) {
        findings.push({
          ...
          severity: 'P3',
          fix: `Переписать alt так, чтобы он укладывался в ${min}–${max} слов.`,
        });
      }
    });
