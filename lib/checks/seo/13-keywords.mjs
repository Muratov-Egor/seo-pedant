  ...
  run(f, ctx) {
    ...
    if (missingInText.length) {
      findings.push({
        ...
        actual: `нет: ${missingInText.map((k) => k.word).join(', ')}`,
    ...
    if (alts.length) {
      ...
      if (!inAlt) {
        findings.push({
          ...
          expected: `хотя бы один alt с ключевым словом (${ctx.keywords.all.map((k) => k.word).join(', ')})`,
          actual: `ни в одном из ${alts.length} alt нет ключевых слов`,
