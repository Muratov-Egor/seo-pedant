...
import { hasAnyStem } from '../../text.mjs';
...
export default {
  ...
  run(f, ctx) {
    ...
    const missingInText = ctx.keywords.all.filter((k) => !hasAnyStem(text, k.stems));
    if (missingInText.length) {
      ...
      findings.push({
        ...
        fix: 'Упомянуть ключевые слова в тексте страницы — или задать верные keywords в config/pages.json.',
      });
    }

    const alts = f.html.images.map((i) => i.alt).filter(Boolean);
    if (alts.length) {
      const inAlt = ctx.keywords.all.some((k) => alts.some((alt) => hasAnyStem(alt, k.stems)));
      if (!inAlt) {
        findings.push({
          ...
          severity: 'P3',
          fix: 'Добавить ключевое слово страницы хотя бы в один alt изображения.',
        });
      }
    }
